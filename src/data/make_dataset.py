...
for f in files:
    ...
    participant = f.replace(data_path, "").split("-")[0].replace("\\", "")
    label = f.replace(data_path, "").split("-")[1]
    category = f.replace(data_path, "").split("-")[2].rstrip("123").rstrip("_MetaWear_2019")
...
data_resampled["set"] = data_resampled["set"].astype(int) #changes "set" to integer
data_resampled.info()
...
#Debugging (unrelated to project)
'''
import sys
print(sys.executable)

Then, copy file path and along with the file path type -m pip install *package*
